...
func set_tile(id, initial_position):
	...
	var posibleTiles = _get_tiles_with_id(Globals.singletons["AssetLoader"].tiles, abs(id))
	var variant  = randi() % len(posibleTiles);

	_description = posibleTiles[variant];
	if (abs(id) != abs(_id)): $SpineSprite.set_new_state_data(_description.spine_data);
	_id = id;
